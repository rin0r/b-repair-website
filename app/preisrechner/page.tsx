...
const SamsungLogo = () => (
  <svg viewBox="0 0 260 55" className="w-28 h-auto">
    <text x="2" y="46" fontSize="50" fontWeight="500" fontFamily="system-ui,sans-serif" fill="currentColor" letterSpacing="-1">Samsung</text>
  </svg>
);
...
export default function PreisrechnerPage() {
  ...
  /* Available repairs for the selected model */
  const availableRepairs = useMemo(() => {
    if (!modelRow) {
      // OnePlus: no price data → all repairs "Auf Anfrage"
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 bg-brand-surface">
      <div className="max-w-xl mx-auto px-4 sm:px-6">
        ...
        {step === 1 && (
          <div>
            <h2 className="font-headline text-2xl text-brand-primary text-center mb-6">
              Welche Marke?
            </h2>
            <div className="grid grid-cols-2 gap-4">
              {BRANDS.map(({ id, name, Logo, color }) => (
                <button
                  ...
                  className="group p-7 rounded-2xl bg-white border-2 border-brand-border hover:border-brand-accent hover:shadow-md transition-all flex flex-col items-center gap-4"
                >
        ...
        {step === 2 && brandId && (
          <div>
            ...
            <h2 className="font-headline text-2xl text-brand-primary mb-5">
              Welches Modell?
            </h2>
            ...
            <div className="max-h-96 overflow-y-auto rounded-2xl border border-brand-border bg-white">
              {brandId === "oneplus" ? (
                onePlusFiltered.length > 0 ? (
                  ...
                  <div className="px-5 py-8 text-center text-brand-gray font-sans text-sm">
                ...
                brandConfig[brandId]?.series.map((series) => {
                  ...
                  return (
                    <div key={series.label}>
                      {/* Series header (Samsung only has multiple series) */}
        ...
        {step === 3 && (
          <div>
            ...
            <h2 className="font-headline text-2xl text-brand-primary mb-1">Was ist kaputt?</h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              {availableRepairs.map(({ key, label, Icon, price }) => (
                <button
                  ...
                  className="p-5 rounded-2xl bg-white border-2 border-brand-border hover:border-brand-accent hover:shadow-md transition-all text-left flex flex-col gap-3"
                >
        ...
        {step === 4 && (
          <div>
            {/* Summary breadcrumb */}
            <p className="font-sans text-xs text-brand-gray text-center mb-6">
              <span style={{ color: brandMeta?.color }} className="font-bold">{brandMeta?.name}</span>
              {" · "}{modelName}{" · "}{repairLabel}
            </p>

            {/* Price card */}
            <div className="bg-white rounded-2xl border border-brand-border p-8 text-center shadow-sm mb-6">
